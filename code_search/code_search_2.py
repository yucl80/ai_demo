...
# 初始化 Elasticsearch
es = Elasticsearch(os.getenv("ELASTICSEARCH_URL", "http://localhost:9200"), verify_certs=False, ssl_show_warn=False)
...
# 初始化 FastAPI 应用
app = FastAPI(title="Code Search API", description="An API for indexing and searching code", version="1.0.0")
...
def extract_functions(code: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        logger.warning("Failed to parse code for function extraction.")
    return functions
...
async def bulk_index_code(code_files: List[CodeFile]):
    ...
    try:
        success, _ = await bulk(es, generate_actions())
        logger.info(f"Bulk indexed {success} documents")
    except Exception as e:
        logger.error(f"Error in bulk indexing: {str(e)}")

async def search_code(query: str, language: Optional[str] = None, top_k: int = 10) -> List[SearchResult]:
    """搜索代码"""
...
@app.post("/index")
async def index_code_api(code_file: CodeFile, background_tasks: BackgroundTasks):
    """API endpoint for indexing code"""
    background_tasks.add_task(index_code, code_file.file_path, code_file.content)
