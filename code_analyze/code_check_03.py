...
def get_analysis_prompt(code_snippet, analysis_type, context="", call_chain=""):
    ...
    prompts = {
        ...
        "issues_and_suggestions": f"请详细分析以下代码可能存在的问题，并按严重性分级（高、中、低），例如潜在的bug、性能问题或代码冗余。请尽可能简洁，。\n\n上下文信息：{context}\n\n函数调用链：{call_chain}\n\n```python\n{code_snippet}\n```",
        "security": f"请详细分析以下代码可能存在的安全漏洞，并按严重性分级（高、中、低），例如潜在的SQL注入、跨站点脚本（XSS）漏洞或其他常见的安全问题。请尽可能简洁。\n\n上下文信息：{context}\n\n函数调用链：{call_chain}\n\n```python\n{code_snippet}\n```",
        "code_quality": f"请详细评估以下代码的质量和可读性，并按严重性分级（高、中、低），包括代码结构、命名约定和代码风格，以确保代码的一致性和可维护性。请尽可能简洁。\n\n上下文信息：{context}\n\n函数调用链：{call_chain}\n\n```python\n{code_snippet}\n```",
        # "refactoring": f"请详细分析以下代码并提供具体的重构建议，帮助优化代码的结构和性能，使其更加清晰和高效，并按严重性分级（高、中、低）列出建议。请尽可能详细，并提供实际案例。\n\n上下文信息：{context}\n\n函数调用链：{call_chain}\n\n```python\n{code_snippet}\n```",
        "all": f"请详细分析以下代码，包括功能理解、可能存在的问题或改进建议、安全漏洞、代码质量评估，并按严重性分级（高、中、低）列出。请尽可能简洁。\n\n上下文信息：{context}\n\n函数调用链：{call_chain}\n\n```python\n{code_snippet}\n```"
    }
