...
class Settings(BaseSettings):
    model: str = "/home/test/llm-models/chatglm3-ggml.bin"
    num_threads: int = os.cpu_count() - 2 or 0
...
async def get_embeddings(request: EmbeddingRequest):
    ...
    if request.model == "bge-large-zh-v1.5":
        if bge_zh_model is None:
            bge_zh_model = LlamaCppEmbeddings(
                model_path="/home/test/llm-models/bge-large-zh-v1.5-q4_k_m.gguf", verbose=False
            )
        embeddings = bge_zh_model.embed_documents(request.input)
    else:
        if bge_m3_model is None:
            bge_m3_model = SentenceTransformer(EMBEDDING_PATH, device="cpu").eval()

        embeddings = bge_m3_model.encode(sentences=request.input, show_progress_bar=False)
