...
@app.get('/search2')
async def emb(q: str = Query(..., min_length=1, max_length=512, title='query'), keys: str = Query(..., title='keys'), top: int = Query(5, ge=1, le=10, title='top')):
    try:
        st = time.perf_counter()
        sentences = keys.split(',')
        query_embedding = s_model.encode(q)
        embeddings = s_model.encode(sentences)
        hits = semantic_search(query_embedding, embeddings, top_k=top)
        et = time.perf_counter() - st
        print("search time:", et)
        hits = hits[0]
        print(hits)
        ...
        return sentences[hits[0]['corpus_id']]
    except Exception as e:
        logger.error(e)
        return {'status': False, 'msg': e}, 400
...
@app.get('/search3')
async def emb(q: str = Query(..., min_length=1, max_length=512, title='query'), keys: str = Query(..., title='keys'), top: int = Query(5, ge=1, le=10, title='top_k')):
    try:
        st = time.perf_counter()
        sentences = keys.split(',')
        stmp = np.append(corpus,sentences,axis=0)
        query_embedding = s_model.encode(q)
        embeddings = s_model.encode(sentences)
        tmp = np.append(corpus_embeddings,embeddings,axis=0)

        hits = semantic_search(query_embedding, tmp, top_k=top)
        hits = hits[0]    

        et = time.perf_counter() - st
        print("search time:", et)
        for hit in hits:
            print(stmp[hit['corpus_id']], "(Score: {:.4f})".format(hit['score']))
        return stmp[hits[0]['corpus_id']]
    except Exception as e:
        logger.error(e)
        return {'status': False, 'msg': e}, 400
