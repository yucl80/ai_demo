...
def create_review_request(lang,old_code, new_code, call_chain):
    ...
    prompt = f"""请审查以下代码变更，根据调用关系分析最终的影响，关注点包括代码变更点及最终影响.请严格按照以下要求进行代码评审:
    1. 确定文件是否包含重大逻辑变更。
    2. 生成差异变更的简要摘要以及最终影响的简要摘要，不超过 50 个字。不包括第一步的结果。  
    3. 请务必用中文以简短的语句描述变更(change), 最终的关联影响(affected),请以JSON Object Array的格式回复 :\n
        旧代码:
        ```{lang}
        {old_code}
        ```\n
