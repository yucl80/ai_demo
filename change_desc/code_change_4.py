...
def create_change_request(lang, old_code, new_code):
    prompt = f"""
    Act as a code reviewer, I will provide you class function has change,
please review the code change according to the following requirements:
Briefly summarize the content of the diff change in Chinese, no more than 100 words,
 just summarize the content of the change.
    代码如下：
        旧代码:
        ```{lang}
        {old_code}
