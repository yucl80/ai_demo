    ...
    def process_image(self, image_path):
        ...
        try:
            ...
            result_image = self.current_image.copy()
             ...
             # 检测直线和折线
            horizontal_lines, vertical_lines = self.detect_orthogonal_lines(self.processed_image)
            polylines = self.detect_polylines(self.processed_image)
            
            # 连接线段
            connected_horizontals = self.connect_line_segments(horizontal_lines)
            connected_verticals = self.connect_line_segments(vertical_lines)
            
            # 绘制水平线
            for line_group in connected_horizontals:
                for segment in line_group:
                    cv2.line(
                        result_image,
                        tuple(segment.start_point),
                        tuple(segment.end_point),
                        (0, 255, 0),
                        2
                    )
            
            # 绘制垂直线
            for line_group in connected_verticals:
                for segment in line_group:
                    cv2.line(
                        result_image,
                        tuple(segment.start_point),
