...
class FlowchartRecognizer:
    ...
    def detect_line_type(self, points):
        """检测线条类型"""
        try:
            if len(points) == 2:
                return "straight"
            ...
            # 判断类型
            if all(abs(d - 90) < 10 or d < 10 or d > 170 for d in directions):
                return "orthogonal"
            ...
            if len(angles) == 2:
                if all(80 <= a <= 100 for a in angles):
                    return "double_polyline"
                    
            if len(points) > 4:
                return "complex_polyline"
                
            return "polyline"
            
        except Exception as e:
            logging.error(f"Error in detect_line_type: {str(e)}")
            return "straight"
...
def main():
    ...
    # 处理图像
    image_path = 'D:\\workspaces\\python_projects\\ai_demo\\vl\\e1.png'  # 替换为实际的图像路径
