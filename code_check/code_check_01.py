...
def get_analysis_prompt(code_snippet, analysis_type, context=""):
    """
    生成用于代码分析的提示。
...
def code_analysis(code_snippet, analysis_type="all", context=""):
    """
    根据指定的分析类型分析代码片段。
    ...
    :return: 分析结果文本
    """
    prompt = get_analysis_prompt(code_snippet, analysis_type, context)
...
def multi_step_analysis(code_snippet, context=""):
    """
    使用多步骤分析对代码片段进行综合分析。
    ...
    for step in steps:
        results[step] = code_analysis(code_snippet, analysis_type=step, context=context)
